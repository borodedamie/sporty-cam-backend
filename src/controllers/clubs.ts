...
export const getClubHighlights = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    if (!id) {
      return res.status(400).json({ status: "failed", message: "Club id is required" });
    }
    ...
    const { data, error, count } = await builder;
    if (error) {
      logger.error("getClubHighlights error:", error);
      return res.status(400).json({ status: "failed", message: error.message });
    }
    ...
    logger.error("getClubHighlights unexpected error:", err);
