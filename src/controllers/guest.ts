...
// change email to club's email
        ...
        to: data.email,
        subject: "Guest Payment Received – Guest Player Cleared for Today’s Session",
        html: generateGuestAdminEmailHtml(club.name,data.full_name, data.email, club?.guest_fee,data.time_preference,data.preferred_training_day),
        };
        ...
        transporter.sendMail(clubMailOptions, (error: any, info: any) => {
          if (error) logger.error("Error sending club email:", (error as any).message || error);
       ...
       transporter.sendMail(guestMailOptions, (error: any, info: any) => {
        if (error) logger.error("Error sending guest email:", (error as any).message || error);
